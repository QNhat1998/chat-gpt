...
const Sidebar = ({ isOpen, setIsOpen }) => {
  const { theme, chats, setSelectedChat, setTheme, user, navigate } = useApp()
  ...
  return (
    <div className={`${!isOpen && 'max-md:-translate-x-full'} flex flex-col h-screen min-w-72 p-5 dark:bg-linear-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      />
      {/* New Chat Button */}
      <button className='flex justify-center items-center w-full py-2 mt-10 text-white bg-linear-to-r from-[#4A56F7] to-[#3D81F6] text-sm rounded-md cursor-pointer'>
